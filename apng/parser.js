...
const { APNG, Frame } = require('./structs');
...
function parseAPNG(buffer) {
    ...
    eachChunk(bytes, (type, bytes, off, length) => {
        ...
        switch (type) {
            ...
            case 'IDAT':
                ...
                break;
            case 'IEND':
                postDataParts.push(Buffer.from(subBuffer(bytes, off, 12 + length).buffer));
                break;
            default:
                preDataParts.push(Buffer.from(subBuffer(bytes, off, 12 + length).buffer));
        }
    });
    ...
    const preBlob = Buffer.concat(preDataParts),
        postBlob = Buffer.concat(postDataParts);
    ...
    apng.frames.forEach(frame => {
        ...
        bb = [
            Buffer.from(PNGSignature.buffer),
            Buffer.from(makeChunkBytes('IHDR', headerDataBytes).buffer),
            preBlob,
            Buffer.concat(frame.dataParts.reduce((arr, p) => { arr.splice(arr.length, 0, Buffer.from(makeChunkBytes('IDAT', p).buffer)); return arr; }, [])),
            postBlob
        ];
        ...
        //bb.push(postBlob);
        frame.imageData = Buffer.concat(bb); //new Blob(bb, { 'type': 'image/png' });
